...
import { MIT_BindingConflicts$, MIT_ShowRebindConfirm$ } from "bindings"
...
export const MIT_RebindConfirm = () =>
{
    ...
    const bindingConflicts = useValue(MIT_BindingConflicts$);

    // translation handling. Translates using locale keys that are defined in C# or fallback string here.
    const { translate } = useLocalization();

    function ProcessBindingConflicts () : ReactNode
    {
        let lines: ReactNode[] = [];
        if (bindingConflicts.conflicts !== undefined) {
            for (let i=0; i<bindingConflicts.conflicts.length; i++) 
            {
                lines[i] = <>{translate(bindingConflicts.conflicts[i].mapNameLocaleKey, bindingConflicts.conflicts[i].mapNameFallback) + " - " + translate(bindingConflicts.conflicts[i].actionNameLocaleKey, bindingConflicts.conflicts[i].actionNameLocaleKey)}</>
            }
        }
                ...
                <Portal>
                    <Panel
                        ...
                        <PanelSection className={styles.section}>
                            <FormattedParagraphs>
                                {translate("MoveIt.TEXT_LABEL[MConflictMessage]", locale["MoveIt.TEXT_LABEL[MConflictMessage]"])}
                                {ProcessBindingConflicts()}
                            </FormattedParagraphs>
                        {/* </PanelSection>
                        <PanelSection className={styles.section}> */}
